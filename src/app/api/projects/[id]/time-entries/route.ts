...
export async function GET(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  // Récupérer toutes les time entries avec les infos utilisateur
  ...
  // Pour les WORKER, ne montrer que leurs propres entrées
  const filteredEntries = auth.role === "WORKER" 
    ? entries.filter(e => e.userId === auth.userId)
    : entries;
